...
def trainvalisplit(spark, username):
    ...
    selected_user_ids = interactions_count.orderBy('num_interactions').limit(500).select('user_id').rdd.flatMap(lambda x: x).collect()
    ...
    unique_msid = train_filtered.select("recording_msid").distinct()
    ...
    train_R_sorted.show()
    train_R_sorted.write.partitionBy("user_id").mode('overwrite').parquet(f'/user/{username}/1004-project-2023/train_inter_FS.parquet')
    ...
    valid_true_sorted.write.partitionBy("user_id").mode('overwrite').parquet(f'/user/{username}/1004-project-2023/vali_inter_FS.parquet')
